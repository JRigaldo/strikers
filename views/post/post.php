<?php $title="Publication"; ?>
<?php
    dd($params);
?>
?>
<main>
    <section class="section__banner section__banner-post">
        <a href="#"><h4 class="section__title">Extinction Rebellion</h4></a>
            <article class="post">
                <div class="article__category">
                    <span>egypte</span><span>caire</span>
                </div>
                <div class="article__image">
                    <img src="images/pictures/image-1.jpg" alt="">
                </div>
                <h1 class="article__title"><span class="title-date">06.07.2022</span>Sur la place Tarir</h1>
                <div class="article__content padding-5">
                    <h2>Extinction Rebellion</h2>
                    <p>Lorem ipsum dolor sit amet, consectetur adipiscing elit. Cras interdum erat eros, id feugiat orci
                        laoreet et.
                        Quisque dapibus ultricies arcu vitae convallis.</p>
                </div>
                <div class="article__footer flex-center space-between">
                    <button><a href="#" class="article__footer--link">https://xrlausanne.ch</a></button>
                    <button><a href="#" class="article__footer--btn">Participer</a></button>
                    <button><a href="#" class="article__footer--btn--icon"><img src="images/icons/share-icon.svg"
                                                                                alt=""></a></button>
                </div>
            </article>
    </section>
</main>